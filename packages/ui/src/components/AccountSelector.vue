...
// Note: Store import will be handled by the consuming application
...
interface Props {
  // Props to be passed from parent when store is not available
  accounts?: any[]
  activeAccount?: any
  loading?: boolean
  groupedAccounts?: any[]
...
// For shared component, these would need to be passed as props or injected
...
const selectedAccount = computed({
  ...
  set: (value: string) => {
    // In shared component, emit event for parent to handle
    emit('accountChanged', value)
  }
})

const accountStatusClass = computed(() => {
  if (!props.activeAccount) return 'unknown'
  
  const status = props.activeAccount.status
...
const accountStatusText = computed(() => {
  if (!props.activeAccount) return 'No account selected'
...
const hasViolations = computed(() => {
  return props.activeAccount ? props.activeAccount.violations.some((v: any) => !v.resolved) : false
})

const violations = computed(() => {
  return props.activeAccount ? props.activeAccount.violations.filter((v: any) => !v.resolved) : []
})
...
async function refreshAccount() {
  if (!props.activeAccount || refreshing.value) return
  
  refreshing.value = true
  try {
    // Emit event for parent to handle store action
    emit('accountChanged', props.activeAccount.id)
  } catch (error) {
