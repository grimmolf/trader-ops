"""
Charles Schwab OAuth2 Authentication

Handles the OAuth2 flow for Schwab API access including token management,
...
class SchwabCredentials(BaseModel):
    ...
    client_id: str = Field(..., description="Schwab app key/client ID")
    client_secret: str = Field(..., description="Schwab app secret")
    redirect_uri: str = Field(default="https://127.0.0.1:8182", description="OAuth redirect URI")
    
    class Config:
        str_strip_whitespace = True


class SchwabTokens(BaseModel):
    """OAuth2 tokens from Schwab API"""
...
class SchwabAuth:
    ...
    def __init__(self, credentials: SchwabCredentials):
        ...
        # Schwab API URLs
        self.auth_base_url = "https://api.schwabapi.com/v1/oauth"
        self.api_base_url = "https://api.schwabapi.com/trader/v1"
        
        logger.info("Initialized Schwab OAuth2 authentication")
    ...
    def generate_authorization_url(self, state: Optional[str] = None) -> str:
        ...
        params = {
            ...
            "scope": "readonly",  # Start with readonly, can be expanded
            "state": state
        }
        
        auth_url = f"{self.auth_base_url}/authorize?" + urlencode(params)
    ...
    async def exchange_code_for_tokens(self, authorization_code: str) -> SchwabTokens:
        ...
        Returns:
            SchwabTokens: Token pair for API access
        """
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await client.get(
                f"{self.api_base_url}/accounts/accountNumbers",
                headers=headers
            )
            response.raise_for_status()
